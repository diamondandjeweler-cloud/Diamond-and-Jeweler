    ...
    <div className="flex items-start justify-between gap-4 mb-4">
      <div>
        <div className="flex items-baseline gap-2">
          ...
          {typeof count === 'number' && <span className="text-sm text-ink-400 dark:text-fg-muted">{count}</span>}
        </div>
...
export function SubHeader({ title, count }: { title: string; count: number }) {
  return (
    <div className="flex items-baseline gap-3 mb-4 mt-2">
      <h3 className="font-display text-lg text-ink-800 dark:text-fg">{title}</h3>
      <span className="text-sm text-ink-400 dark:text-fg-muted">{count}</span>
    </div>
  )
}
