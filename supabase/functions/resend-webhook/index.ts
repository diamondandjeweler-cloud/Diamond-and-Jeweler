...
const corsHeaders = { 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Headers': 'content-type,svix-id,svix-timestamp,svix-signature' }
...
serve(async (req) => {
  ...
  if (webhookSecret) {
    ...
    // No secret configured (dev/staging) — process without signature check.
    const event = (await req.json().catch(() => null)) as ResendEvent | null
