...
import { listMarketRates, insertMarketRate, updateMarketRate, deleteMarketRate } from '../../../data/repositories/marketRates'
...
export default function MarketRatePanel() {
  ...
  async function saveEdit() {
    if (!editing) return
    const { error } = await updateMarketRate(editing, { ...draft, snapshot_date: new Date().toISOString().slice(0, 10) })
  ...
  return (
    <div>
      ...
      {creating && (
        <div className="bg-surface border dark:border-border rounded p-3 mb-4">
          <h4 className="text-sm font-semibold mb-2 dark:text-fg">New market rate</h4>
      ...
      {rows.length === 0 ? (
        ...
        <table className="w-full text-sm dark:text-gray-300">
          ...
          <tbody>
            {rows.map((r) => (
              editing === r.id ? (
                <tr key={r.id} className="border-b dark:border-border align-top">
                ...
                <tr key={r.id} className="border-b dark:border-border last:border-0">
                  ...
                  <td className="text-xs text-fg-muted">{r.snapshot_date}</td>
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
      <input value={draft.job_title} onChange={(e) => set('job_title', e.target.value)}
        placeholder="Job title" className="border dark:border-border rounded px-2 py-1 col-span-2" />
      <input value={draft.location ?? ''} onChange={(e) => set('location', e.target.value)}
        placeholder="Location" className="border dark:border-border rounded px-2 py-1" />
      <select value={draft.experience_level ?? 'mid'}
        onChange={(e) => set('experience_level', e.target.value)}
        className="border dark:border-border rounded px-2 py-1">
      ...
      <input type="number" value={draft.min_salary ?? ''}
        ...
        placeholder="Min" className="border dark:border-border rounded px-2 py-1" />
      <input type="number" value={draft.max_salary ?? ''}
        onChange={(e) => set('max_salary', parseInt(e.target.value, 10) || 0)}
        placeholder="Max" className="border dark:border-border rounded px-2 py-1" />
      <input type="number" value={draft.median_salary ?? ''}
        onChange={(e) => set('median_salary', parseInt(e.target.value, 10) || 0)}
        placeholder="Median" className="border dark:border-border rounded px-2 py-1" />
      <input value={draft.currency} onChange={(e) => set('currency', e.target.value)}
        placeholder="Currency" className="border dark:border-border rounded px-2 py-1" maxLength={3} />
    </div>
  )
}
