...
serve(async (req) => {
  try {
    ...
    await reportError(e, { fn: 'buy-points' })
    throw e
  }
})
...
async function handler(req: Request): Promise<Response> {
  ...
  // bill-creation body below (double-click / retry), so a single intent does not
  // create two pending purchases + two Billplz bills. The webhook's own CAS
  // guards remain authoritative against a double point-grant.
  const idemKey = req.headers.get('Idempotency-Key')
  const result = await withIdempotency(db, idemKey, auth.userId, 'buy-points', async () => {
    ...
    if (!pkg) return { _status: 404, _body: { error: `Unknown package_id: ${packageId}` } }
    ...
    const { data: purchase, error: insErr } = await db
      ...
      .select('id')
      .single()
    if (insErr || !purchase) return { _status: 500, _body: { error: insErr?.message ?? 'Insert failed' } }
    ...
    const { error: linkErr } = await db.from('point_purchases')
      ...
      .eq('id', purchase.id)
    if (linkErr) return { _status: 500, _body: { error: `Bill link failed: ${linkErr.message}` } }
