                ...
                <div className="p-5 flex flex-wrap items-center justify-between gap-4">
                  <div>
                    ...
                    <div className="text-xs text-ink-500 dark:text-gray-400 mt-0.5">
                      ...
                      <span>
                        {s.format
                          ? t(`hrDash.format.${s.format}`, { defaultValue: s.format })
                          : '—'}
                      </span>
                    </div>
                  </div>
                  <div className="flex gap-2 flex-wrap items-center">
                    {s.meeting_url ? (
                      ...
                      <Button
                        size="sm"
                        variant="secondary"
                        onClick={() => onCreateMeetingLink(s.interview_id)}
                      >
                        {t('hrDash.createMeetingLink')}
                      </Button>
                    )}
                    <Button
                      size="sm"
                      onClick={() => void onCompleteInterview(s.interview_id, s.match_id, true)}
                    >
                      {t('hrDash.markHired')}
                    </Button>
                    <Button
                      size="sm"
                      variant="secondary"
                      onClick={() => void onCompleteInterview(s.interview_id, s.match_id, false)}
                    >
                      {t('hrDash.notHired')}
                    </Button>
                  </div>
                        ...
                        <div className="text-xs text-ink-500 dark:text-gray-400 mt-0.5 flex items-center gap-2">
                          ...
                          <Badge tone="green">
                            {t('hrDash.percentMatch', {
                              pct: Math.round(p.compatibility_score ?? 0),
                            })}
                          </Badge>
                        </div>
                      </div>
                      {schedulingId !== p.id && (
                        <Button size="sm" onClick={() => onStartScheduling(p.id)}>
                          {t('hrDash.scheduleInterview')}
                        </Button>
                      )}
                    </div>
                    {schedulingId === p.id && (
                      <div className="mt-5 grid md:grid-cols-3 gap-3 pt-5 border-t border-ink-100 dark:border-gray-700">
                        ...
                        />
                        <Select
                          label={t('hrDash.formatLabel')}
                          value={format}
                          onChange={(e) => onSetFormat(e.target.value as typeof format)}
                        >
                        ...
                        <div className="flex items-end gap-2">
                          <Button onClick={() => void onConfirmSchedule(p.id)} className="flex-1">
                            {t('hrDash.confirm')}
                          </Button>
                          <Button variant="secondary" onClick={onCancelScheduling}>
                            {t('common.cancel')}
                          </Button>
                        </div>
                      </div>
