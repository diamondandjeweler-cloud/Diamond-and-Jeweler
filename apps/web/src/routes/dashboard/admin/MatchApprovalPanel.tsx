  ...
  return (
    <div>
        ...
        <button
          ...
          className={`ml-4 px-4 py-2 rounded text-sm font-medium transition-colors ${
            autopilot
              ...
              : 'bg-gray-200 dark:bg-surface-2 text-gray-700 dark:text-fg hover:bg-gray-300 dark:hover:bg-gray-600'
          }`}
        >
      ...
      {loading ? <ListSkeleton rows={5} variant="row" /> : rows.length === 0 ? (
        <p className="text-sm text-fg-muted">No matches pending approval.</p>
      ) : (
        <div className="space-y-2">
          {rows.map((m) => {
            ...
            return (
              <div key={m.id} className="bg-surface border dark:border-border rounded-lg overflow-hidden">
                {/* Summary row */}
                <div className="flex items-start gap-3 p-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {m.roles?.industry && (
                        <span className="text-xs text-fg-muted border dark:border-border rounded px-1.5 py-0.5">
                          {m.roles.industry}
                        </span>
                      )}
                    </div>
                    {m.roles?.description && (
                      <p className="text-xs text-fg-muted mt-0.5 line-clamp-1">{m.roles.description}</p>
                    )}
                    ...
                    <div className="flex gap-6 mt-2 text-xs text-fg-muted">
                      <span>
                ...
                {expanded === m.id && (
                  <div className="border-t dark:border-border bg-gray-50 dark:bg-gray-900 p-4 space-y-3 dark:text-gray-300">
                    ...
                    {Object.keys(tags).length > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-fg-muted mb-1.5">All talent strengths</p>
