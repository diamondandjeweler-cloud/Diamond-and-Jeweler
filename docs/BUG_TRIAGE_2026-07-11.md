...
### ✅ i18n gaps fixed in `beb0979` (71 keys, ms + zh added to all 3 locales, key-parity verified)
- `Kiosk.tsx` (customer-facing) — order-flow strings now wrapped in `t()` under `restaurant.kiosk.*`.
- `Referrals.tsx` — user-facing strings wrapped under `referrals.*`.
- points-spend confirm dialogs — the missing title keys (`hmDash.urgentConfirmTitle`/`redeemConfirmTitle`, talent twins, `common.confirm`) added.
- ⚠️ Translations are **machine-generated** (spot-checked correct — e.g. "Place order" → "Buat pesanan" / "下单") but should get a **native-speaker review pass** before any marketing-critical use.

### ⛔ Still genuinely need you (one item)
...
---
*Generated by an autonomous bug-hunt loop (find → adversarial verify → gated fix → live deploy). What's left is a single product fact only the owner knows.*
