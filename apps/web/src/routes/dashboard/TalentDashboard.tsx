    ...
    <div>
      ...
      {(extractionStatus === 'pending' || extractionStatus === 'processing') && (
        <div className="mb-6 rounded-xl border border-brand-200 bg-brand-50 px-4 py-3 flex items-start gap-3">
          <svg
            className="animate-spin h-5 w-5 text-brand-500 mt-0.5 shrink-0"
            viewBox="0 0 24 24"
            fill="none"
            aria-hidden
          >
          ...
          <div className="text-sm text-brand-900">
            ...
            <div className="text-brand-800 mt-0.5">{t('talentDash.analysingBody')}</div>
          </div>
        </div>
      )}
      ...
      {extractionStatus === 'failed' && (
        <div className="mb-6">
          <Alert tone="red">
            ...
            <Link to="/talent/profile" className="underline font-semibold">
              {t('talentDash.openYourProfile')}
            </Link>{' '}
      ...
      {profileGaps.length > 0 && <ProfileCompletenessBar gaps={profileGaps} />}
            ...
            {items.map((m) => {
              ...
              return (
                <OfferCard
                  ...
                  onPickSlot={(slot) =>
                    pendingProposal && void pickInterviewSlot(m.id, pendingProposal.id, slot)
                  }
                  onDeclineProposal={() =>
                    pendingProposal && void declineInterviewProposal(m.id, pendingProposal.id)
                  }
                  feedbackEntry={
                    talentFeedbackState[m.id] ?? {
                      rating: 0,
                      outcome: '',
                      freeText: '',
                      saving: false,
      ...
      <Card className="mt-8 border-2 border-amber-400 bg-amber-50/40">
        <div className="p-6">
            ...
            <div className="mb-3 rounded-lg border-2 border-amber-300 bg-surface p-4">
              ...
              <div className="mt-0.5 text-xs text-fg-muted flex gap-2 flex-wrap">
                ...
                {urgentResult.role.work_arrangement && (
                  <>
                    <span>·</span>
                    <span className="capitalize">{urgentResult.role.work_arrangement}</span>
                  </>
                )}
              </div>
          ...
          <Button onClick={handleUrgentJobSearch} disabled={urgentBusy}>
            {urgentBusy
              ? t('talentDash.searching')
              : t('talentDash.urgentButton', { cost: URGENT_COST })}
          </Button>
        </div>
      </Card>
        ...
        <Card className="mt-8 border-dashed border-accent-500">
          <div className="p-6 text-center">
            ...
            <p className="text-sm text-fg-muted mb-4">
              ...
              <br />
              {pointsBalance != null
                ? t('talentDash.extraPayOrSpendBalance', {
                    cost: POINTS_PER_EXTRA,
            ...
            <div className="flex flex-wrap items-center justify-center gap-2">
              {pointsBalance != null && pointsBalance < POINTS_PER_EXTRA ? (
                ...
                >
                  {redeemingExtra
                    ? t('talentDash.redeeming')
                    : t('talentDash.extraUsePoints', { cost: POINTS_PER_EXTRA })}
                </Button>
              )}
            ...
            {unlockMsg && (
              <div
                className={`mt-3 text-xs ${unlockMsg.tone === 'green' ? 'text-emerald-700' : 'text-red-700'}`}
              >
                {unlockMsg.text}
              </div>
            )}
